...
describe('<Event /> component', () => {
  ...
  test('render events with details hidden', () => {
    expect(EventWrapper.state('showDetails')).toBe(false);
  });

  test('render event detials', () => {
    EventWrapper.setState({ showDetails: true });
    expect(EventWrapper.find('.event-details')).toHaveLength(1);
  });

  test('render event with details shown', () => {
    EventWrapper.setState({ showDetails: true });
    expect(EventWrapper.find('.event-details').children()).toHaveLength(5);
  });

  test('show event details on clicking details button', () => {
    EventWrapper.setState({ showDetails: false});
    EventWrapper.find('.details-button').simulate('click');
    expect(EventWrapper.state('showDetails')).toBe(true);
  });

  test('hide event details on click details button', () => {
    EventWrapper.setState({ showDetails: true});
    EventWrapper.find('.details-button').simulate('click');
    expect(EventWrapper.state('showDetails')).toBe(false);
  });
